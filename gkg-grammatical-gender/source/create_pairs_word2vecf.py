...
def save_checkpoint(checkpoint_file, line_number):
    with open(checkpoint_file, 'w') as f:
        json.dump({'line_number': line_number}, f)

def load_checkpoint(checkpoint_file):
    if os.path.exists(checkpoint_file):
        with open(checkpoint_file, 'r') as f:
            checkpoint = json.load(f)
            return checkpoint['line_number']
    return 0
...
def print_pairs(lang, vocab, to_male_dict=None, to_female_dict=None, to_lemma_dict=None, to_neut_dict=None, to_options_dict=None):
    ...
    num_of_lines = count_lines(args.input)

    with codecs.open(args.input, 'r', 'utf-8') as f, codecs.open(args.output, 'w', 'utf-8') as f_out:
        for j, l in enumerate(tqdm(f, total=num_of_lines)):
            # if j < start_line:
            #     continue
            ...
            # if j % 1000 == 0:  # Save checkpoint every 1000 lines
            #     save_checkpoint(checkpoint_file, j)
...
def create_vocab():
    ...
    with open(args.input, 'r', encoding="utf-8") as f:
        ...
        f.seek(0)
        for l in tqdm(f, total=num_of_lines):
